...
class RegistroFormulario extends StatefulWidget {
  ...
  const RegistroFormulario({super.key, required this.userType});
...
class _RegistroFormularioState extends State<RegistroFormulario> {
  ...
  final _placaController =
      TextEditingController(); // Campo placa para conductor
  ...
  Future<void> _guardarDatosEnFirestore(UserCredential userCredential) async {
    final userType =
        widget.userType; // Obtener el tipo de usuario ('cliente' o 'conductor')
    ...
    // Para conductor
      ...
      await _firestore
          ...
          .set({
            ...
            "correo": _emailController.text.trim(),
            "contraseña": _passwordController.text.trim(),
            "conectado": true, // Campo para saber si está conectado
            "placa": _placaController.text
                .trim(), // Campo de placa para conductor
          });
    }
  }
  ...
  Future<void> _registrarUsuario() async {
    if (_formKey.currentState!.validate()) {
      try {
        ...
        // Redirigir a la pantalla correspondiente (Cliente o Conductor)
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => widget.userType == 'cliente'
                ? const MapaCliente()
                : const MapaConductor(),
          ),
        );
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Campo Placa (solo para conductor)
                  if (widget.userType == 'conductor') ...[
                    SizedBox(
                      height: screenHeight * 0.01,
                    ), // Espacio antes del campo
                    TextFormField(
